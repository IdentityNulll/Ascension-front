...
export const incrementQuest = createAsyncThunk(
  "quests/increment",
  async (id, thunkAPI) => {
    try {
...
export const decrementQuest = createAsyncThunk(
  "quests/decrement",
  async (id, thunkAPI) => {
    try {
...
const questSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getQuests.rejected, (state) => {
        state.isLoading = false;
      })
      ...
      .addCase(incrementQuest.fulfilled, (state, action) => {
        ...
        const index = state.quests.findIndex(
          (q) => q._id === action.payload.quest._id,
        );
