...
const Records = () => {
  ...
  if (isLoading)
    return (
      <div className="text-center p-8 text-slate-400">Loading Records...</div>
    );
      ...
      <div className="glass flex-1 rounded-xl overflow-hidden flex flex-col min-h-[500px]">
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-3">
          {records.map((record) => (
            <div
              key={record._id}
              className="flex items-center justify-between p-4 bg-surface/50 rounded-lg border border-white/5 hover:bg-white/5 transition-colors"
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-2 rounded-lg ${record.xpChange >= 0 ? "bg-success/20 text-success" : "bg-danger/20 text-danger"}`}
                >
                  {record.xpChange >= 0 ? (
                    <TrendingUp size={20} />
                  ) : (
              ...
              <div
                className={`font-bold text-lg ${record.xpChange >= 0 ? "text-success" : "text-danger"}`}
              >
                {record.xpChange >= 0 ? "+" : ""}
                {record.xpChange} XP
              </div>
