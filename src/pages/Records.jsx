...
const Records = () => {
  ...
  const categoryOptions = ["all", "quest", "shop", "tax", "rule", "system"];

  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      const matchSearch = record.details
        .toLowerCase()
        .includes(searchQuery.toLowerCase());

      const matchXp =
        xpFilter === "all" ||
        (xpFilter === "positive" && record.xpChange > 0) ||
        (xpFilter === "negative" && record.xpChange < 0) ||
        (xpFilter === "neutral" && record.xpChange === 0);
      ...
      const matchDate =
        !dateFilter ||
        new Date(record.createdAt).toISOString().slice(0, 10) === dateFilter;

      return matchSearch && matchXp && matchCategory && matchDate;
    });
  }, [records, searchQuery, xpFilter, categoryFilter, dateFilter]);
  ...
  if (isLoading && records.length === 0) {
    return (
      <div className="text-center p-8 flex justify-center">
        <ActionLoader size={40} className="text-white" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-wider text-slate-200 uppercase drop-shadow-md">
            Audit Log
          </h1>
          <p className="text-slate-400 mt-2">
            Immutable history of your existence.
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
          ...
          {["all", "positive", "negative", "neutral"].map((f) => (
            <button
              key={f}
              onClick={() => setXpFilter(f)}
              className={`px-5 py-2 rounded-full text-xs font-bold whitespace-nowrap capitalize transition-all ${
                xpFilter === f
        ...
        <select
          ...
          className="bg-surface/80 border border-white/10 rounded-xl px-4 py-2 text-sm text-slate-200 focus:outline-none"
        >
      ...
      <div className="glass p-6 md:p-10 rounded-3xl">
        <div className="space-y-4">
          {filteredRecords.map((record) => (
            <div
              key={record._id}
              className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-5 bg-background/50 rounded-2xl border border-white/5 hover:border-white/20 transition-all gap-4 shadow-inner group"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="text-[10px] uppercase px-2 py-1 rounded-md bg-white/5 text-slate-400 border border-white/5">
              ...
              <span
                className={`text-sm font-black px-4 py-2 rounded-xl whitespace-nowrap shadow-sm border border-white/5 ${
                  record.xpChange >= 0
                    ? "bg-success/10 text-success"
                    : "bg-danger/10 text-danger"
                }`}
