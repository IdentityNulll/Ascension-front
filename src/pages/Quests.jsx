...
const Quests = () => {
  ...
  const categories = useMemo(() => {
    const cats = new Set(
      quests.map((q) => q.category?.toLowerCase() || "other"),
    );
  ...
  const filteredQuests = useMemo(() => {
    ...
    return quests.filter(
      (q) => (q.category?.toLowerCase() || "other") === activeFilter,
    );
  }, [quests, activeFilter]);

  if (isLoading && quests.length === 0) return <Loading />;

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h1 className="text-4xl font-black tracking-wider text-primary uppercase">
          Quests
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredQuests.map((quest) => (
          <div
            key={quest._id}
            className="glass rounded-2xl flex flex-col overflow-hidden border border-white/5 hover:border-primary/30 transition-all hover:-translate-y-1 group"
          >
            <div className="p-5 flex-1 relative">
              {/* Delete Button (appears on hover or always on mobile) */}
              <button
                onClick={() =>
                  window.confirm("Delete this quest?") &&
                  dispatch(deleteQuest(quest._id))
                }
              ...
              <h3 className="font-bold text-xl mb-2 text-slate-100 pr-8">
                {quest.title}
              </h3>
        ...
        {filteredQuests.length === 0 && (
          <div className="col-span-full p-12 flex flex-col items-center justify-center text-slate-400 glass rounded-2xl border-2 border-dashed border-white/10">
            ...
            <p className="text-sm opacity-70">
              Focus your path by creating one.
            </p>
          </div>
        )}
      </div>
