...
const Shop = () => {
  ...
  if (isLoading && items.length === 0) return <div className="text-center p-8 text-secondary font-medium animate-pulse">Loading Shop...</div>;

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-wider text-secondary uppercase">Black Market</h1>
        ...
        <div className="bg-surface/80 backdrop-blur border border-secondary/30 px-6 py-3 rounded-xl flex items-center gap-3">
          ...
          <span className="text-secondary font-bold tracking-wider text-xl">{user?.xp || 0} XP</span>
        </div>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {items.map(item => (
          <div key={item._id} className="glass p-6 rounded-2xl border border-white/5 hover:border-secondary/30 transition-all hover:-translate-y-1 flex flex-col group relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-6 z-10">
              ...
              <button 
                onClick={() => window.confirm('Remove item from shop?') && dispatch(deleteShopItem(item._id))}
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-6 bg-surface/50 rounded-xl border border-white/5 mb-6 z-10">
              <span className="text-sm font-medium text-slate-400 mb-1">Cost</span>
              <span className="text-secondary font-black text-3xl">{item.price} XP</span>
            </div>
        ...
        {items.length === 0 && (
          <div className="col-span-full p-12 flex flex-col items-center justify-center text-slate-400 glass rounded-2xl border-2 border-dashed border-white/10">
            ...
            <p className="text-sm opacity-70">Add infinite rewards in the Create page.</p>
          </div>
        )}
      </div>
    </div>
