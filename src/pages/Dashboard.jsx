  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-h-full">
      <header className="flex flex-col gap-2">
        <h1 className="text-4xl font-black tracking-wider text-primary">
          DASHBOARD
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass p-6 rounded-2xl flex flex-col gap-4 border-t-4 border-t-secondary transition-transform hover:-translate-y-1">
          ...
          <p className="text-4xl font-bold">
            {user?.xp || 0} <span className="text-xl text-secondary">XP</span>
          </p>
        </div>
        ...
        <div className="glass p-6 rounded-2xl flex flex-col gap-4 border-t-4 border-t-warning transition-transform hover:-translate-y-1">
          ...
          <p className="text-4xl font-bold">
            {user?.streak || 0}{" "}
            <span className="text-xl text-warning">Days</span>
          </p>
        </div>
      </div>
        ...
        <div className="glass p-8 rounded-2xl flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            {records.slice(0, 6).map((record) => (
              <div
                key={record._id}
                className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-colors gap-3"
              >
                <p className="text-sm text-slate-200">{record.details}</p>
                <span
                  className={`text-xs font-bold px-3 py-1.5 rounded-lg whitespace-nowrap ${record.xpChange >= 0 ? "bg-success/20 text-success" : "bg-danger/20 text-danger"}`}
                >
                  {record.xpChange >= 0 ? "+" : ""}
                  {record.xpChange} XP
                </span>
            ...
            {records.length === 0 && (
              <div className="text-slate-500 italic py-8 text-center text-sm border-2 border-dashed border-white/10 rounded-xl">
        ...
        <div className="glass p-8 rounded-2xl flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            {quests.slice(0, 5).map((quest) => (
              <div
                key={quest._id}
                className="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between hover:border-primary/30 transition-colors"
              >
                <div className="flex flex-col">
                  <p className="font-bold text-slate-200">{quest.title}</p>
                  <p className="text-xs text-primary font-medium mt-1 inline-block uppercase tracking-wider">
                    {quest.category}
                  </p>
                </div>
            ...
            {quests.length === 0 && (
              <div className="text-slate-500 italic py-8 text-center text-sm border-2 border-dashed border-white/10 rounded-xl">
                No active quests. Time to set some goals!
              </div>
            )}
          </div>
        </div>
